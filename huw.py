...
class HUWebshop(object):
    ...
    def recommendations(self, count):
        ...
        # http response, bevat hvld recommendations en welke
        resp = requests.get(self.recseraddress+"/"+session['profile_id']+"/"+str(count))
        # als resp code klopt(200)
        if resp.status_code == 200:
            # recs is dan de 4 gerecommende products
            recs = eval(resp.content.decode())
            # queryfilter filter op die 4 recommended products
            queryfilter = {"_id": {"$in": recs}}
            # querycursor geeft de gegevens die horen bij die 4 recommended products
            querycursor = self.database.products.find(queryfilter, self.productfields)
            # resultlist wordt lijst met alle 4 de recommended prod, met de prices en images etc
